...
export const copyComponent = (component: string) => {
  ...
  const destPath = path.resolve('apps/todo-app/components/ui');
...
export const processNonComponentFile = (filePath: string) => {
  ...
  const todoAppDir = path.resolve('apps/todo-app/utils/gluestack-utils');
  try {
    fileOps.copyDir(packagesDir, todoAppDir);
  } catch (error) {
