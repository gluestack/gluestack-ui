...
export const MobileSidebarMenu = ({ onItemClick }: MobileSidebarMenuProps) => {
  ...
  return (
    <div className="flex flex-col gap-6 pt-4">
      {headerItems.map((section) => (
        <div key={section.title} className="flex flex-col gap-2">
          <h3 className="text-sm font-semibold text-foreground/70 px-2">
            {section.title}
          </h3>
          <div className="flex flex-col gap-1">
            {section.items.map((item) => {
              const logoElement =
                mounted && colorMode === 'dark' ? item.logoDark : item.logo;
