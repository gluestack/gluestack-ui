...
export const Note = ({ children, ...props }: any) => {
  return (
    <Box className="p-4 flex border-1 border-border border rounded-md mb-6 mt-8">
      <HStack className="w-full items-center justify-start flex" space="md">
        <Box className="flex justify-center items-center h-14 w-14 rounded-full bg-muted">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="25"
            viewBox="0 0 24 25"
            className="stroke-foreground fill-muted-foreground stroke-2"
          >
            <path
        ...
        <Box className="max-w-fit max-h-fit flex-1 text-sm gap-2.5">
          ...
          <Box className="[&>*]:!text-foreground [&>*]:!text-sm [&>*]:!leading-7 [&>*]:mb-0 [&>*]:!tracking-medium [&>*]:!font-sans ">
            {children}
          </Box>
        </Box>
      </HStack>
