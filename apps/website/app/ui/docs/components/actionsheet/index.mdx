...
It is React Native's [Pressable](https://reactnative.dev/docs/pressable) component, created using [react-native-reanimated's](https://docs.swmansion.com/react-native-reanimated/) `Animated.createAnimatedComponent` function to add animation to the component. It fades in and out with `FadeIn`/`FadeOut` animations when the Actionsheet opens and closes. You can override these using the `entering` and `exiting` props.

#### ActionsheetContent

It inherits all the properties of [react-native-reanimated's](https://docs.swmansion.com/react-native-reanimated/) `Animated.View` component, and slides in from the bottom (`SlideInDown`) and out to the bottom (`SlideOutDown`) when the Actionsheet opens and closes. You can override these using the `entering` and `exiting` props.
