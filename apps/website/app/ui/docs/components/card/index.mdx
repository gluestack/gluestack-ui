import { CodePreviewer } from '@/components/custom/code-previewer';
import { Card } from '@/components/ui/card';
import { Heading } from '@/components/ui/heading';
import { Text } from '@/components/ui/text';

import { Meta } from '@/components/custom/meta';

  <Meta pageTitle="Card" pageDescription="Build beautiful interfaces with the gluestack-ui Card component. This React Native card offers a clean, modern design for any project. Perfect for seamless card design UI integration." />

# Card

Build beautiful interfaces with the gluestack-ui Card component. This React Native card offers a clean, modern design for any project. Perfect for seamless card design UI integration.


### This is an illustration of Card component.

  

  
  
  <CodePreviewer
  code={`function Example() {
  return (
    <Card size="{{size}}" variant="{{variant}}" className="m-3">
      <Heading size="md" className="mb-1">
        Quick Start
      </Heading>
      <Text size="sm">Start building your next project in minutes</Text>
    </Card>
  )
}`}
  argTypes={{
  "variant": {
    "control": {
      "type": "select"
    },
    "options": [
      "elevated",
      "outline",
      "ghost",
      "filled"
    ],
    "defaultValue": "elevated"
  },
  "size": {
    "control": {
      "type": "select"
    },
    "options": [
      "sm",
      "md",
      "lg"
    ],
    "defaultValue": "md"
  }
}}
  reactLive={{ Card, Heading, Text }}
/>