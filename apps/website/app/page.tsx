...
export default function Example() {
  const [messages, setMessages] = React.useState([
    {
      ...
      role: 'assistant',
      content:
        'Hi! I can help you with various tasks. What would you like to work on today?',
    },
  ]);
  ...
  const renderMessage: ListRenderItem<UIMessage> = ({
    item: message,
    index,
  }) => (
  ...
  const handleSubmit = ({ text }: { text: string }) => {
    ...
    setTimeout(() => {
      const aiReply = {
        ...
        content:
          'Hello! . This is a fake generated response. In a real app, this would come from Grok, GPT, or any AI model using Vercel AI SDK.',
      };
  ...
  return (
    <Box className="h-[600px] web:pb-5 bg-background">
      <Conversation className="flex-1">
        <ConversationContent messages={messages} renderItem={renderMessage} />
        <ConversationScrollButton />
      </Conversation>
