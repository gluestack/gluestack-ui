...
// NativeWind v5 metro.config.js — inlined for the same reason.
const NATIVEWIND_V5_METRO_CONFIG_EXPO = `const { getDefaultConfig } = require('expo/metro-config');
...
const NATIVEWIND_V5_METRO_CONFIG_RN_CLI = `const { getDefaultConfig, mergeConfig } = require('@react-native/metro-config');
const { withNativewind } = require('nativewind/metro');
...
// NativeWind v5 babel.config.js for RN CLI — no nativewind/babel needed in v5.
const NATIVEWIND_V5_BABEL_CONFIG_RN_CLI = `module.exports = {
...
// UniWind metro.config.js — inlined for the same reason as babel: not yet in main-v4-alpha cache.
const UNIWIND_METRO_CONFIG_EXPO = `const { getDefaultConfig } = require('expo/metro-config');
...
const UNIWIND_METRO_CONFIG_RN_CLI = `const { getDefaultConfig } = require('@react-native/metro-config');
const { withUniwindConfig } = require('uniwind/metro');
...
// UniWind babel.config.js — inlined to avoid dependency on cached repo having the file.
// The templates cache uses main-v4-alpha which may not have the v5 templates yet.
const UNIWIND_BABEL_CONFIG_EXPO = `module.exports = function (api) {
  api.cache(true);
...
const UNIWIND_BABEL_CONFIG_RN_CLI = `module.exports = {
  presets: ['module:@react-native/babel-preset'],
  plugins: [
    [
      'module-resolver',
...
const InitializeGlueStack = async ({
  ...
  try {
    ...
    console.log(`\n\x1b[1mInitializing gluestack-ui v5...\x1b[0m\n`);
    ...
    log.success(
      `\x1b[32mDone!\x1b[0m Initialized \x1b[1mgluestack-ui v5\x1b[0m in the project`
    );
  } catch (err) {
  ...
  switch (projectType) {
    ...
    case config.expoProject:
      ...
      if (config.style === 'nativewind') {
        ...
        files.push('postcss.config.*', 'react-native-css-env.d.ts');
      } else if (config.style === 'uniwind') {
        files.push('uniwind-types.d.ts');
      }
      break;
    case config.reactNativeCLIProject:
      files = [
