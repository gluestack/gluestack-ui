...
const gluestackDeps = [
  ...
  export default function withGluestackUI(nextConfig: any = {}) {
    ...
    try {
      rootExactDependencyList = getExactDependenciesFromNodeModules(currDir, reactNativeDeps);
    } catch (e) {
      // Silently fail if node_modules scanning fails
    }
    ...
    if (workspaceRoot) {
      try {
        const workspaceRootDeps = getDependenciesFromNodeModules(workspaceRoot, gluestackDeps);
        const workspaceRootExactDeps = getExactDependenciesFromNodeModules(workspaceRoot, reactNativeDeps);
        
        parentDependencyList = [...parentDependencyList, ...workspaceRootDeps];
        parentExactDependencyList = [...parentExactDependencyList, ...workspaceRootExactDeps];
      } catch (e) {
