...
export default function Factory<P>(
  ...
  const StyledComponent: any = styled(
    Component,
    transformTheme(componentTheme)
  );
  return React.forwardRef(({ children, _state, ...props }: any, ref: any) => {
    if (componentTheme) {
      // eslint-disable-next-line react-hooks/rules-of-hooks
      const inlineProps: any = usePropResolution(props);
      // const resolvedPropForGluestack = usePropResolution(props);

      return (
        <StyledComponent ref={ref} {...inlineProps}>
          {children}
        </StyledComponent>
      );
    } else {
