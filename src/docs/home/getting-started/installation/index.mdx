...
```bash
yarn add @gluestack-ui/core@alpha @gluestack-ui/utils@alpha react-native-svg @gluestack/ui-next-adapter@alpha react-native-web react-native
```

OR

```bash
npm i @gluestack-ui/core@alpha @gluestack-ui/utils@alpha react-native-svg @gluestack/ui-next-adapter@alpha react-native-web react-native
```

It's recommended to use the `--legacy-peer-deps` flag due to peer dependency resolution issues.

```bash
npm i @gluestack-ui/core@alpha @gluestack-ui/utils@alpha react-native-svg @gluestack/ui-next-adapter@alpha react-native-web react-native --legacy-peer-deps
```

> **For React Native CLI:** Make sure to install `react-dom` as well as it is required by some of the Gluestack UI components. `npm i react-dom`

> **Animation Libraries:** In gluestack-ui v4, we primarily use `react-native-reanimated` (~4.1.0) for most animated components (Accordion, Drawer, Modal, Menu, Popover, Toast). Some components still use `@legendapp/motion` (Tooltip, ActionSheet, Select). The CLI automatically installs the required dependencies for your project.
