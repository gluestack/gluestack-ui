...
import { UtilsLink, OverlayLink, ToastLink, GluestackUIProviderLink } from './utils-link';
...
```bash
yarn add @expo/html-elements @legendapp/motion dom-helpers react-aria react-native-svg react-native-web react-stately tailwind-variants nativewind
```
...
```bash
npm i @expo/html-elements @legendapp/motion dom-helpers react-aria react-native-svg react-native-web react-stately tailwind-variants nativewind
```
...
```bash
npm i @expo/html-elements @legendapp/motion dom-helpers react-aria react-native-svg react-native-web react-stately tailwind-variants nativewind --legacy-peer-deps
```

```bash
npm i -D @react-native/assets-registry @types/react-native tailwindcss@3 postcss autoprefixer typescript --legacy-peer-deps
```


### Step 3: Copy gluestack-utils

Copy all the contents of the gluestack-utils folder from the <UtilsLink /> into your project's `@/utils/gluestack-utils` directory.

### Step 4: Copy required components
 
 - Copy all the contents of the Overlay component folder from the{" "}<OverlayLink />{" "}into your project's `@/components/ui/overlay` directory.
 - Copy all the contents of the Toast component folder from the{" "}<ToastLink />{" "}(except docs and examples folder) into your project's `@/components/ui/toast` directory.
...
#### Step 5.3: Configure GluestackUIProvider
