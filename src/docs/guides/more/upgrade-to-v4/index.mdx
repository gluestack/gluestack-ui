...
- **Automatic CLI Upgrade**: Simple upgrade process with `npx gluestack-ui@alpha upgrade` command
...
⚠️ **Important:** v4 introduces a new shadcn-like color token system. You need to update your `tailwind.config.js` with the new color definitions.
...
All gluestack-ui v4 components — including Tooltip, ActionSheet, and Select — now use `react-native-reanimated` for animations. `@legendapp/motion` is no longer part of the dependency tree and is not installed by the CLI.

> **Note:** `react-native-reanimated` (and its `react-native-worklets` peer) is automatically installed and configured by the CLI.
...
> Looking to upgrade to NativeWind v5 or UniWind? See the [Upgrade to v5 guide](/ui/docs/guides/more/upgrade-to-v5).
