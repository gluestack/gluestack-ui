...
```jsx
  ...
  // For Showing the Image that can be clicked to open the ImageViewer
  <Pressable onPress={() => setVisible(true)}>
  <Image source={{ uri: Images[0].url }} className="w-[200px] h-[200px]" resizeMode="contain" />
  </Pressable>
  </>
);
}
