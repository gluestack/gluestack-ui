...
#### Basic Unitools Link
The below example will run for both Expo and Next.js projects. For installation steps, refer to the [Installation](https://unitools.geekyants.com/packages/link/) section of ```@unitools/link```.
  ...
  <CodePreview
    ...
    metaData={{
      code: `
        ...
        function Home() {
          return (
            <UnitoolsLink href="https://gluestack.io/">
              <Text>gluestack</Text>
            </UnitoolsLink>
          );
        }
...
> Note: For the Next.js project replace the ```<Text>``` with the ```<p>```