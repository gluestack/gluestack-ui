...
> Note: **Our responsive grid component is based on a 12-column grid layout. It follows the CSS grid system on the web and the flexbox layout on native devices. Since the grid layout is only supported on the web, passing the grid-cols-* and col-span-* classNames inside _extra is recommended for the grid component to work effortlessly on both web and native. Our grid component currently does not support rowSpan.**
