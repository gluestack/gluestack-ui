...
In this guide we will learn how we can create unstyled library using `@gluestack-ui`.

## Default Components

By default, when you use `@gluestack-ui/themed`, you get plain, unstyled components.

But, it also comes with a package called `gluestack-ui/config`, which has ready-made styles for all the components. So, you can choose these styles if you like.

And, the cool part is, you can even create your own unique style if you want to give these components a personal touch.

> Note: If you want to create your own design system with these components, it's advisable to refer to the customization guide available [guide](https://ui.gluestack.io).
...
When we use the `GluestackUIProvider`, it doesn't come with any predefined styles. So, by default, all the components you get from `@gluestack-ui/themed` are unstyled.
