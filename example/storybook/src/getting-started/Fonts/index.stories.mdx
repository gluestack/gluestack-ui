...
## Advance concepts for fonts in gluestack-ui

In gluestack-ui, components are styled using `styled` provided by `@dank-style/react` package, [check here]('https://dank.style/').
...
You can use this `styled` function which has the font plugin added to it to style your components. For gluestack-ui components, we have used this styled function to style the components.
