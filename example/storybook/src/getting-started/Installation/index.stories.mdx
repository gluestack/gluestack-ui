...
We can add a new component using npx `npx npx gluestack-ui add <package-name>`. For example, to add the button component package, you can run the following command:
...
For Next.js Projects update \_app.tsx with below code:
...
```bash
 npx npx gluestack-ui add <component-name>
```
