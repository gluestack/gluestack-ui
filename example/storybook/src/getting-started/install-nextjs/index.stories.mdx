...
```bash
npx gluestack-ui@latest@latest init
```
...
We can add a new component using npx `npx gluestack-ui@latest@latest add <package-name>`. For example, to add the button component package, you can run the following command:

```bash
npx gluestack-ui@latest@latest add button
```
