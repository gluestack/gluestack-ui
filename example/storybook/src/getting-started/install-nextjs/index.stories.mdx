...
<Meta title="Getting Started/InstallInNextjs" />
...
It's also recommended to set up your server-side rendering (SSR) correctly. To do this, you will need to use the `flush()` function exported by the library and call it in your `_document.tsx` file. So, for Next.js projects we require `@gluestack/ui-next-adapter` as a dependency and update `_document.tsx` with the below code:
