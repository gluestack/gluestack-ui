...
Dank Style exports a StyledProvider component that can be used to configure your components. This is useful when you want to define some aliases, tokens and global style at a global level throughout your app. You can define them in `dank.config.js` file and pass it to the StyledProvider.

In order to create config you need to use `createConfig` function from dank-style. It takes three arguments `aliases`,`tokens` and `global style`. Apart from config StyledProvider also accepts colorMode, which denotes the current active colorMode of the application and globalStyleInjector, which is a function that can be used to inject global styles.
