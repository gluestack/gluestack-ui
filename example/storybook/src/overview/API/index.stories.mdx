---
title: API | Dank
description: A brief about all the API that you get from Dank.
---
...
1. **descendantStyle**
   ...
   descendantStyle is an array of string that denotes the descendants of that component that should respond to parents state. This is useful for creating a complex styles that is dependent on the component that this prop is passed in.
...
2. **ancestorStyle**
   ...
   ancestorStyle is an array of string that denotes the ancestors of that component. This is useful for creating complex styles that is dependent on the ancestor component.
...
3. **resolveProps**
   ...
   resolveProps is an array of string that denotes the props that should be resolved. This is useful for props that are not passed into the style property rather goes in as a prop. For example, `placeholderTextColor` in TextInput.
...
Apart from config StyledProvider also accepts colorMode, which denotes the current active colorMode of the application.
...
Dank Style exports a AsForwarder component that can be used to configure your components. This is useful when you want to render the component passed in as prop with styling added by styled function.
...
`propertyResolver` is an object that lets you define a custom resolver for a property. This is useful when you want to define a custom resolver for a property.
...
`mediaQueries` is an object that lets you define a custom media queries(breakpoints) for your app. These breakpoints can be used in the styling to write responsive styles.
...
These `colorMode` styles are applied when your colorMode gets `true`, these colorModes are passed in `colorMode` prop of the StyledProvider.
...
`sx` prop is a prop that lets you write inline styles for the component. It is useful when you want to write some custom styles for a component.
...
Dank Style provides utility props that can be used to style the component. These props get converted to sx prop internally. There is a simple rule when writing utility props, always start your props with a media query token first if you are styling for responsive design, and after that just keep writing the conditions with a hyphen in between texts and the suggestions from the typescript will take care of the rest.
...
<StyledInput md-py={'$4'} lg-py={'$5'} dark-bg={'$gray800'} />;
```
