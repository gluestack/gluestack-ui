import { Canvas, Meta, Story } from '@storybook/addon-docs';

<Meta title="components/Button" />

import { AccessibleButton } from './Button';
import { AppProvider, CodePreview, Table } from '@gluestack/design-system';

# Button

A Button component is used to create clickable elements that perform an action when pressed.

<AppProvider>
  <CodePreview
    showComponentRenderer={true}
    showArgsController={false}
    metaData={{
      code: ` 
      function App () {
        return (
         <Button>
            <Button.Text>Button</Button.Text>
          </Button>
    )
    }`,
      scope: { Button: AccessibleButton },
      argsType: {},
    }}
  />
</AppProvider>

<br />

## Features

- Keyboard support for actions.
- Support for hover, focus and active states.

## Installation

```jsx
yarn add @universa11y/button
```

## Usage

```jsx
import { createButton } from '@universa11y/button';
const Button = createButton({
  Root,
  Text,
  Group,
  GroupSpacer,
  Spinner,
});
```

These are the styled components that we need to pass in the creator API:

### Root

```jsx
const Root = styled(
  Pressable,
  {
    ...
  },
);
```

### Text

```jsx
import { styled } from '@dank-style/react';
const Text = styled(
  Text,
  {
    ...
  },
);
```

### Group

```jsx
import { styled } from '@dank-style/react';
const Group = styled(
  View,
  {
    ...
  },
);
```

### Spinner

```jsx
import { styled } from '@dank-style/react';
const Spinner = styled(
  ActivityIndicator,
  {
    ...
  },
);
```

### GroupSpacer

```jsx
import { styled } from '@dank-style/react';
const GroupSpacer = styled(
  View,
  {
    ...
  },
);
```

## Anatomy

```jsx
export default () => (
  <Button>
    <Button.Text />
    <Button.Spinner />
  </Button>
);
```

## API Reference

### Root

Contains all button related layout style props and actions.
It inherits all the properties of React Native's [Pressable](https://reactnative.dev/docs/pressable#props) component.

<Table>
  <Table.THead>
    <Table.TR>
      <Table.TH>Prop</Table.TH>
      <Table.TH>Type</Table.TH>
      <Table.TH>Default</Table.TH>
      <Table.TH>Description</Table.TH>
    </Table.TR>
  </Table.THead>
  <Table.TBody>
    <Table.TR>
      <Table.TD>isLoading</Table.TD>
      <Table.TD>bool</Table.TD>
      <Table.TD>false</Table.TD>
      <Table.TD>When true, its loading state activates.</Table.TD>
    </Table.TR>
    <Table.TR>
      <Table.TD>isHovered</Table.TD>
      <Table.TD>bool</Table.TD>
      <Table.TD>false</Table.TD>
      <Table.TD>To manually set hover to the button.</Table.TD>
    </Table.TR>
    <Table.TR>
      <Table.TD>isPressed</Table.TD>
      <Table.TD>bool</Table.TD>
      <Table.TD>false</Table.TD>
      <Table.TD>To manually set pressable state to the button.</Table.TD>
    </Table.TR>
    <Table.TR>
      <Table.TD>isFocused</Table.TD>
      <Table.TD>bool</Table.TD>
      <Table.TD>false</Table.TD>
      <Table.TD>To manually set focused state to the button.</Table.TD>
    </Table.TR>
    <Table.TR>
      <Table.TD>isDisabled</Table.TD>
      <Table.TD>bool</Table.TD>
      <Table.TD>false</Table.TD>
      <Table.TD>To manually set disable to the button.</Table.TD>
    </Table.TR>
  </Table.TBody>
</Table>

### Text

Contains all text related layout style props and actions.
It inherits all the properties of React Native's [Text](https://reactnative.dev/docs/text#props) component.

### Group

Contains all group related layout style props and actions.
It inherits all the properties of React Native's [View](https://reactnative.dev/docs/view#props) component.

<AppProvider>
  <Table>
    <Table.THead>
      <Table.TR>
        <Table.TH>Prop</Table.TH>
        <Table.TH>Type</Table.TH>
        <Table.TH>Default</Table.TH>
        <Table.TH>Description</Table.TH>
      </Table.TR>
    </Table.THead>
    <Table.TBody>
      <Table.TR>
        <Table.TD>direction</Table.TD>
        <Table.TD>'column' or 'row'</Table.TD>
        <Table.TD>'row'</Table.TD>
        <Table.TD></Table.TD>
      </Table.TR>
      <Table.TR>
        <Table.TD>isDisabled</Table.TD>
        <Table.TD>bool</Table.TD>
        <Table.TD>false</Table.TD>
        <Table.TD>
          When Table.TRue, Table.THis will disable all Table.THe buttons in a
          ButtonGroup
        </Table.TD>
      </Table.TR>
      <Table.TR>
        <Table.TD>isAttached</Table.TD>
        <Table.TD>bool</Table.TD>
        <Table.TD>Table.TRue</Table.TD>
        <Table.TD>
          When attached, boTable.TH buttons will be attached to each
          oTable.THer.
        </Table.TD>
      </Table.TR>
      <Table.TR>
        <Table.TD>reversed</Table.TD>
        <Table.TD>bool</Table.TD>
        <Table.TD>false</Table.TD>
        <Table.TD>To reverse Table.The order of components</Table.TD>
      </Table.TR>
    </Table.TBody>
    <Table.TFoot>
      <Table.TR>
        <Table.TD>reversed</Table.TD>
        <Table.TD>bool</Table.TD>
        <Table.TD>false</Table.TD>
        <Table.TD>To reverse Table.The order of components</Table.TD>
      </Table.TR>
    </Table.TFoot>
  </Table>
</AppProvider>

### GroupSpacer

Contains all spacer related layout style props and actions.
It inherits all the properties of React Native's [View](https://reactnative.dev/docs/view#props) component.

### Spinner

Contains all spinner related layout style props and actions.
It inherits all the properties of React Native's [ActivityIndicator](https://reactnative.dev/docs/activityindicator#props) component.

## Example

<AppProvider>
  <CodePreview
    showComponentRenderer={true}
    showArgsController={false}
    metaData={{
      code: ` 
      function App () {
        return (
        <Button>
          <Button.Text>Button</Button.Text>
        </Button>
    )
    }`,
      scope: { Button: AccessibleButton },
      argsType: {},
    }}
  />
</AppProvider>

## Accessibility

### Keyboard

- Tab: Moves focus to the next focusable element.
- `Shift + Tab`: Moves focus to the previous focusable element.
- Enter: Triggers the button's action.

### Screen Reader

- VoiceOver: When the button is focused, the screen reader will announce the button's label and its current state.

## Dependencies

- `@universa11y/utils`
- `@react-native-aria/focus`
- `react-native-svg`
