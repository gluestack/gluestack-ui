...
import { AddIcon, CheckIcon, CloseIcon, InfoIcon } from './AllIcons';
...
<AppProvider>
  <CodePreview
    ...
    metaData={{
      code: `
        <Icon as={CalendarDaysIcon} color="$red500" {...props}/>
      `,
      transformCode: (code) => {
...
```bash
npx gluestack-ui@latest update icons
```
  ...
  <CodePreview
    ...
    metaData={{
      ...
      <VStack space="md" alignItems='center'>
        ...
        <Icon as={ChromeIcon} color="$primary500" />
        <Icon as={InstagramIcon} size="xl" color="$green500"/>
        <Icon as={FacebookIcon} sx={{ h: "$8", w: "$8", color: 'red' }} />
      </VStack>
`,
      transformCode: (code) => {
        return transformedCode(code);
