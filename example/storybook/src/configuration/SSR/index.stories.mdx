...
It is also recommended to attach either the `gs-dark` or `gs-light` class for default color mode.

This will ensure that the appropriate color mode is applied from the server side and prevent any visual inconsistencies during the initial page load.
