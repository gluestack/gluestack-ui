...
Dank supports server-side rendering (SSR) by providing a **`flush`** function that can be used to flush out any styles generated during the server-side render process. This enables these styles to be added to the HTML document. To use the **`flush`** function, you must import and call it in the **`document.jsx`** or **`_document.tsx`** file, which is a file specific to Next.js that can be customized to modify the document that is sent to the browser.
...
It is also recommended to attach either the `gs-dark` or `gs-light` class for default color mode. This will ensure that the appropriate color mode is applied from the server side and prevent any visual inconsistencies during the initial page load.
