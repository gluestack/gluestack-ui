...
gluestack-style enables you to modify the value resolution for any style property by providing a callback function for any style property in your dank.config file or when creating a component using styled. For example, if you want to add support for RGBA values in the background-color property you can follow the below example.
...
> **Note:** The Property Resolver is locally scoped so if you define it in a components config then it will only modify that property value for that particular component. To apply this functionality globally, define it in the dank.config.js/ts file as shown below.
