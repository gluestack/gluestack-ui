...
React Native has a great StyleSheet API which is optimal for component-based systems. [**`@gluestack-style/react`**](https://v1.gluestack.io/style) leverages it and adds a layer of utility props and constraint based designed tokens on top of it. **gluestack-style** is used as styling engine for [**`@gluestack-ui/themed`**](https://github.com/gluestack/gluestack-ui/blob/main/packages/themed).
