...
Add the font name in the `gluestack.config.js` file in the `fonts` object. If you are confused where to find the fonts config object? Please go through [this](https://v1.gluestack.io/ui/docs/theme-configuration/theme/default-tokens) documentation (specifically the typography section).
