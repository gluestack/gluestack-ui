...
In gluestack-ui, components are styled using `styled` provided by `@glue-style/react` package, [check here]('https://dank.style/').
This package provides a way to add plugins using `createStyled` function.

```bash
import { createStyled, FontResolver } from '@glue-style/react';
...
You can use this `styled` function which has the [font plugin](https://dank.style/docs/plugins/fonts-plugin) added to it to style your components. For gluestack-ui components, we have used this styled function to style the components.
