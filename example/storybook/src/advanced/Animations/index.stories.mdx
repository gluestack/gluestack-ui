...
gluestack-ui leverages the power of the [Animation plugin](https://dank.style/docs/plugins/animation-plugin) from [dank.style](https://dank.style) to provide animations for a more engaging user experience. By default, all of our animated components are built using the [@legendapp/motion](https://legendapp.com/open-source/motion/), which provides declarative animations for React Native.
...
If you are using [@legendapp/motion](https://legendapp.com/open-source/motion/), you can use `styled.Component`, which internally uses `AnimatePresence` from `@legendapp/motion`.

### Why styled.Component and not `AnimatePresence` from `@legendapp/motion`?
...
> If you are using any other animation library, you can use `AnimatePresence` or something similar(which animates component on exit) from your library.
...
You can refer to [animation plugin](https://dank.style/docs/plugins/animation-plugin) documentation for more details.
