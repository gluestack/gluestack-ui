...
1. **Component Barrel Export** - `packages/gluestack-core/src/dropdown/index.tsx`:
```typescript
export * from './creator';
export * from './creator/types';  // if types file exists
export * from './aria';  // if aria exists
```

2. **Main Index Export** - Add to `packages/gluestack-core/src/index.tsx`:
```typescript
export * from './dropdown';
```

3. **Deep Import Files** - Automatically generated by `scripts/generate-barrel-exports.js`:
   - The script runs during `yarn build`
   - It creates files like `packages/gluestack-core/dropdown/creator.ts`
   - These files export from `lib/esm/dropdown/creator`
   - Enables imports like: `import { createDropdown } from '@gluestack-ui/core/dropdown/creator'`

4. **TypeScript Support** - TypesVersions in package.json:
   - Manually add entries to typesVersions for new components
   - Pattern: `"[component]/creator": ["./lib/esm/[component]/creator/index.d.ts"]`
